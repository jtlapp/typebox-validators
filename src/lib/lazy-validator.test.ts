...
class TestObj1 {
  ...
  static readonly validator = new LazyValidator(
    Type.Object({
      delta: Type.Integer(),
  ...
  validate(safely: boolean) {
    TestObj1.validator.validate(this, 'Bad TestObj1', safely);
  }
}
...
describe('LazyValidator getters', () => {
  it('returns the schema', () => {
    const schema = Type.Object({
      delta: Type.Integer(),
      count: Type.Integer({ exclusiveMinimum: 0 }),
    });
    const validator = new LazyValidator(schema);
    expect(validator.schema).toBe(schema);
  });

  it('returns the compiled type', () => {
    const schema = Type.Object({
      delta: Type.Integer(),
      count: Type.Integer({ exclusiveMinimum: 0 }),
    });
    const validator = new LazyValidator(schema);
    const compiledType = validator.compiledType;
    expect(compiledType).toBeInstanceOf(TypeCheck);
    // verify that the compiled type is cached
    expect(validator.compiledType).toBe(compiledType);
  });
});
