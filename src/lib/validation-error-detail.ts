...
export class ValidationErrorDetail implements ValueError {
   ...
   * validation. Otherwise, if the schema did not provide an `errorMessage`,
   ...
   */
  toString(): string {
    if (this.schema.errorMessage !== undefined) {
      const field = this.path ? this.path.substring(1) : '';
      return this.schema.errorMessage.replace('{field}', field);
    }
    return this.path !== ''
